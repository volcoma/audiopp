...
static auto alc_get_max_aux_slots_per_source(ALCdevice* dev) ->std::string
{

    std::stringstream ss;
    int max_aux_sends;
    alcGetIntegerv(dev, ALC_MAX_AUXILIARY_SENDS, 1, &max_aux_sends);

    ss << "OpenALC max auxiliary send slots per source: " << max_aux_sends;
    return ss.str();
}
} // namespace openal
...
device_impl::device_impl(const std::string& id)
{
    ...
    info() << "-- " << openal::alc_get_max_aux_slots_per_source(device_.get());
